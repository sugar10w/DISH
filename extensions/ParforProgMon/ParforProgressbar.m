...
%            import libUtil.progressbar;
           ...
           if iscell(numIterations) % worker
               ...
               write(o.connection, [o.workerID, 0], "uint32", o.ServerName, o.ServerPort);
           else % server
               % - Server constructor
               p = inputParser;
               ...
               % fopen(o.connection);
               o.connection = udpport("datagram", 'EnablePortSharing', true);
               configureCallback(o.connection,"datagram",1,@(x,y) receiver(x,o));
               ...
               o.timer = timer('BusyMode','drop','ExecutionMode','fixedSpacing','StartDelay',p.Results.progressBarUpdatePeriod*2,'Period',p.Results.progressBarUpdatePeriod,'TimerFcn',@(x,y) draw_progress_bar(o));
               start(o.timer);
               o.UserData = {};
           end
       end
       ...
       function increment( o )
           ...
           if mod(o.it, o.stepSize) == 0
               debug('Send it=%d\n',o.it);
               write(o.connection,[o.workerID, o.it], "uint32", o.ServerName, o.ServerPort);
           end
       end
...
function receiver(h, o)
    % [data,count,msg,ip,port] = read(h, 1, 'uint32');
    data = read(h, 1, 'uint32');
    % fprintf("[%d, %d]\n", data.Data(1), data.Data(2));
    if size(data.Data, 2) ~= 2 % error
        debug('Unkown received data from %s:%d with count = %d', data.SenderAddress, data.SenderPort, size(data.Data, 2));
    else
        id = data.Data(1);
        if data.Data(2) == 0 % log in request in worker constructor
            o.workerTable.progress(id) = 0;
            o.workerTable.ip(id) = data.SenderAddress;
            o.workerTable.port(id) = data.SenderPort;
            o.workerTable.connected(id) = true;
            debug('login worker id=%02d with ip:port=%s:%d',id,data.SenderAddress,data.SenderPort);
        else % from worker increment call
            o.workerTable.progress(id) = data.Data(2);
            debug('Set progress for worker id=%02d to %d',id,data.Data(2));
        end
    end
end
...
% fprintf("draw progress\n");
%     import libUtil.progressbar;
pause(0.10);
    progressTotal = sum(o.workerTable.progress) / o.totalIterations;
    % fprintf("%f, %f, %f, %f\n", progressTotal, sum(o.workerTable.progress), o.totalIterations, o.progressTotalOld);
