function H_list = my_propagation_quick3D_init_GPU(N,M, z_list, pixelsize, lambda, method)
% 在使用 my_propagation_quick3D_GPU 前使用，针对z生成H_list矩阵列
% Use before my_propagation_quick3D_GPU to generate H_list for z_list

x=gpuArray(single(1:M));
y=gpuArray(single(1:N));
L0X=pixelsize*M;
L0Y=pixelsize*N;
k=2*pi/lambda;
